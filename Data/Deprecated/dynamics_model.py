...
class CNPModel:
    ...
    def build_model(self):
        ...
        for i, layer_config in enumerate(self.config.dynamics_layers['obs_encoder'], start=1):
        ...
        for i, layer_config in enumerate(self.config.dynamics_layers['decoder'],
                                         start=len(self.config.dynamics_layers['obs_encoder']) + 1):
            layer_config.number = i
            layer = make_layer(layer_config)
            decodes = layer(decodes)
        mu = keras.layers.Dense(name='mu', units=self.state_dims)(decodes)
