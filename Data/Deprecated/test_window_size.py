...
if __name__ == '__main__':
    ...
    #     cur_cnp, prev_cnp, tar, zs = map(lambda xs: np.squeeze(xs, axis=-1), np.split(obs, 4, axis=-1))
    ...
    #         cur, prev, tar, zs = map(lambda xs: np.squeeze(xs, axis=-1), np.split(obs, 4, axis=-1))  # update tar & zs
    #         t += 1
    #
