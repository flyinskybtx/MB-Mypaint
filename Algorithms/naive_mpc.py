...
from Planner.naive_planner import Planner
...
if __name__ == '__main__':
    ...
    planner = Planner(action_space, reward_fn, dynamics, horizon=5, num_particles=200)

    done = False
    while not done:
        canvas = env.to_canvas()
        planner.update(canvas)

        traj, reward = planner.plan()
        action = traj[0].tolist()
