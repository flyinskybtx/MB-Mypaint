...
if __name__ == '__main__':
    ...
    env.reset()
    save_dir = os.path.join(DATA_DIR, 'HWDB/discrete/', author.strip('.pot'))
    writer = JsonWriter(save_dir)
    batch_builder = SampleBatchBuilder()
    #
    for i, sp in enumerate(tqdm(skeleton_paths)):
        wps, action_xy = skeleton_path_to_wps(sp, env_config.xy_grid, env.image_size)
        ...
        obs = env.reset()
        env.position = [wps[0][0], wps[0][1], 0]  # 重置笔刷到skeleton 出发点
        obs_history = [obs]
        ...
        if np.max(target) == 0:  # 目标图像是空图像，有问题
            continue
        for obs, point, action in zip(obs_history, wps, actions):
            ...
            batch_builder.add_values(
                obs=obs,
                actions=action.astype(int).tolist(),
            )
