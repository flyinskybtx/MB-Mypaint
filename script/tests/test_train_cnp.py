...
if __name__ == '__main__':
    ...
    action_embedder = ActionEmbedder(cfg, dtype='float64')
    action_embedder.build(input_shape=(None, 3))

    sims = sorted(glob(os.path.join(DATA_DIR, f'offline/slow_ink', 'Sim*')),
                  key=lambda x: int(x.split('Sim')[-1]))[:10]
    physics = glob(os.path.join(DATA_DIR, f'offline/slow_ink', 'Phy*'))

    train_data = NewCNPData(sims, batch_size=16, num_context=(20, 30),
                            encoder=obs_encoder, embedder=action_embedder, train=True)
    val_data = NewCNPData(sims, batch_size=16, num_context=(20, 30),
                          encoder=obs_encoder, embedder=action_embedder, train=False)
    vis_data = NewCNPData(physics, batch_size=16, num_context=(20, 30),
                          encoder=obs_encoder, embedder=action_embedder, train=False)

    cnp = CNP(cfg)
    cnp.build_graph(input_shape=[(None, None, cfg.latent_size + 15),
                                 (None, None, cfg.latent_size),
                                 (None, cfg.latent_size + 15)])
    cnp.summary()
    ...
    cnp.fit(train_data, epochs=1000,
            ...
            callbacks=[
                ...
                CnpVisCallback(vis_data, obs_encoder, obs_decoder, action_embedder,
