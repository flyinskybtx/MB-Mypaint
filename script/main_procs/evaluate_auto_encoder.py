...
def evaluate_continuity(repr_model):
    ...
    for _ in range(5):
        ...
        latent1 = repr_model.latent_encoder.predict(np.expand_dims(sample1, axis=0))
        latent2 = repr_model.latent_encoder.predict(np.expand_dims(sample2, axis=0))
        ...
        for i in range(0, num_interps + 1):
            latent = latent1 + (latent2 - latent1) * i / num_interps
            delta = repr_model.latent_decode(latent)
            frame.append(delta.squeeze(axis=-1).squeeze(axis=0))
            # plt.imshow(delta.squeeze(axis=-1).squeeze(axis=0))
